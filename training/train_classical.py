"""
train_classical.py — Train GNN + ClassicalLSTMCell (GNN-CLSTM baseline).

Architecture: GNN(G) → θ₀, h₀ → ClassicalLSTMCell × T
No HyperNet — ClassicalLSTMCell has its own learnable gates.

Optimizer: Adam over GNN + cell params.
"""
...
# ---------------------------------------------------------------------------
# Pipeline (no HyperNet)
# ---------------------------------------------------------------------------

def run_pipeline_classical(G, gnn, cell, T, qaoa_p=2, qaoa=None):
    """GNN → ClassicalLSTMCell × T. No HyperNet."""
    if qaoa is None:
    ...
    h_t     = h0.squeeze(0)
    C_t     = torch.zeros(cell.hidden_size)
...
def evaluate(gnn, cell, val_graphs, T, qaoa_p=2, qaoa_cache=None) -> float:
    ...
    with torch.no_grad():
        for G in val_graphs:
            ...
            qaoa, thetas = run_pipeline_classical(G, gnn, cell, T,
...
def train(cfg: dict = CONFIG, resume_ckpt: str | None = None) -> None:
    ...
    dim_h  = cfg['dim_h']
    T      = cfg['T']
    n_qaoa_params = qaoa_p * 2   # 4 for p=2
    ...
    start_epoch    = 1
    best_val_ratio = 0.0
    best_ckpt      = os.path.join(cfg['ckpt_dir'], 'classical_best_p2.pth')
    latest_ckpt    = os.path.join(cfg['ckpt_dir'], 'classical_latest_p2.pth')
    ...
    for epoch in range(start_epoch, cfg['n_epochs'] + 1):
        ...
        for G in train_graphs:
            optimizer.zero_grad()
            qaoa, thetas = run_pipeline_classical(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, default='checkpoints/classical_latest_p2.pth')
